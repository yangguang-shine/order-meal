<template>
    <div class="order-food-item-container flex-row flex-a-center">
        <image class="food-img flex-shrink" :src="orderFoodItem.fullImgPath"></image>
        <div class="food-info flex-col flex-j-between">
            <div class="food-name line1">{{orderFoodItem.foodName}}</div>
			<div class="food-specifica line1">规格：{{specificaText}}</div>
            <div class="food-price flex-row">
                <span class="money-unit">¥</span>
                {{ orderFoodItemSpecifica.currentPrice }}
            </div>
        </div>
        <div class="food-count">×{{ orderFoodItemSpecifica.orderCount }}</div>
        <div class="food-count-price"><span class="money-unit">¥</span>{{ orderFoodItemSpecifica.allCountPrice }}</div>
    </div>
</template>

<script lang="ts" setup>
import { FoodItemI, OrderSpecifaItemI, specificationItemI } from "@/interface/menu";
import {computed} from 'vue'
interface PropsI {
    orderFoodItem: FoodItemI;
    orderFoodItemSpecifica: OrderSpecifaItemI;
}
const props:PropsI = withDefaults(defineProps<PropsI>(), {
    orderFoodItem: {},
    orderFoodItemSpecifica: {},
});
const specificaText = computed(() => {
	return props.orderFoodItemSpecifica.specifa.reduce((str, item) => {
        if (str) {
            str = `${str}、${item.content}`
        } else {
            str = item.content
        }
        return str;
    }, "")
})
// defineProps({
//     orderFoodItem: {
//         type: Object,
//         default: () => ({}),
//     },
//     orderFoodItemSpecifica: {
//         type: Object,
//         default: () => ({}),
//     },
// });
</script>

<style lang="scss" scoped>
.order-food-item-container {
    margin-top: 10rpx;

    padding: 20rpx;
    background-color: #f8f8f8;
    width: 100%;
    box-sizing: border-box;
    .food-img {
        height: 110rpx;
        width: 110rpx;
        // border-radius: 8rpx;
        margin-right: 20rpx;
        // background-color: red;
    }
    .food-info {
        flex: 10;
        width:320rpx;
        font-size: 28rpx;
        height: 110rpx;

        // height: 90rpx;
    }
    .food-name {
        font-size: 28rpx;
        line-height: 40rpx;
        color: #333;
    }
	.food-specifica {
		font-size: 24rpx;
		color: #999;
	}
    .food-price {
        font-size: 24rpx;
        color: #999;
    }
    .food-count {
        flex: 2;
        font-size: 28rpx;
        line-height: 34rpx;
        text-align: center;
        color: #666;
    }
    .food-count-price {
        flex: 2.5;
        font-size: 28rpx;
        text-align: right;
        color: #333;
    }
}
</style>
