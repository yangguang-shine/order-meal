...
1. 分类滚动时，不同手机展示的分类tab不一致（已解决）
2. 首页固定tab时，滚动距离不精准(已解决)
3. 满减信息过长时，超出展示区域（已解决）
4. 不同菜品规格增加或减少时，和预期有很大区别（已解决）
5. 购物车展示无法展示不同的规格（已解决）
6. 购物车多规格时价格不一致（已解决）
7. 搜索菜品时没有图片懒加载（已解决）
8. 店铺水平模式菜品名称超出展示区域（已解决）
9. 增加菜品时，添加按钮的动效结束地址有问题（已解决）
10. 微信浏览器滚动时页面展示的整体的高度会变短或变高，动画结束位置不正确（已解决）
