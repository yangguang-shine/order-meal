<template>
	<div class="order-detail-container">
		...
		<div class="order-info-box">
			<div class="shop-info-box line1">新微信餐厅正餐店</div>
			<div class="order-list-box">
				<div class="order-item" v-for="(foodItem, index) in orderDetail.foodList" :key="index">
					<div class="order-item flex-row">
						<image class="food-img" :src="host + foodItem.imgUrl" mode="scaleToFill"></image>
			...
			<div class="food-amount-info flex-row flex-j-between">
				<div></div>
				<div class="flex-row">
					<div v-if="orderDetail.minusPrice">已优惠</div>
					<div v-if="orderDetail.minusPrice">¥{{ orderDetail.minusPrice }}</div>
					<div>总价</div>
					<div>¥{{ orderDetail.orderAmount }}</div>
				</div>
			</div>
		</div>
